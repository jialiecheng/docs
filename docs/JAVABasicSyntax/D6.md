...
- 创建对象并使用
- 格式：类名 对象名 = new 类名();
- 对象名：其实就是合法的标识符，如果是一个单词所有字母小写，如果多个单词从第二个单词开始首字母大写
#### D:如何使用成员变量呢?
- 对象名.变量名
#### E:如何使用成员方法呢?
- 对象名.方法名(…)
...
```java
 public static void main(String[] args) {
    ...
    tel.brand = "华为";
    tel.price = 5000;
    System.out.println(tel.brand+" "+tel.price);
    tel.call();
    tel.playGame();
    tel.sendMessage();
 }
...
匿名对象可以调用属性但是没有意义，因为调用完没有引用会在堆中变成垃圾
如果需要赋值则要用有名字的对象
```java
  Car c1 = new Car();
  c1.run();  //创建有名字的对象
